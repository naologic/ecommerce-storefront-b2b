...
*/
@Pipe({
  name: 'formTranslateErrorsHTML',
  pure: false // if true, the pipe doesn't update when we change language, but on next request
})
export class FormTranslateErrorsHTMLPipe extends TranslatePipe implements OnDestroy {
  ...
  transform(
    ...
    // -->Process
    this.processValue(formErrors, langPrefix);
   ...
   * Process the values
   */
  public processValue(formErrors, langPrefix): void {
    // -->Set: empty errors
    const values = [];
    ...
    mapValues(formErrors, (err, errIndex) => {
      ...
      // -->Set: errors
      values.push(this.lastLangPaths[lpath].instant);
    });

    // -->Set: value as object with errors
    this.value = values.join(`<br>`);
  }
