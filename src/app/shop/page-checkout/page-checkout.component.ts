...
export class PageCheckoutComponent implements OnInit, OnDestroy {
    ...
    public refresh() {
        ...
        this.eCommerceService.getCheckoutInformation(data$).toPromise().then((res) => {
            // -->Set: addresses
            this.addresses = res?.data?.account?.data?.addresses || [];
            // -->Set: shipping methods
            this.shippingMethods = res?.data?.shippingMethods?.data || []; // appInfo?.shippingMethods || [];
    ...
    public checkOrder(): void {
        ...
        this.checkOrderSubs = this.eCommerceService.verifyCheckout(data$).subscribe(
            (res) => {
                ...
                if (res?.ok) {
                    // -->Set: summary
                    this.summary = res.data?.orderData?.summary
                } else {
                    // -->Show: toaster
                    this.toastr.error(this.translate.instant('ERROR_API_REQUEST'));
                }
