...
import {NaoUserAccessData, NaoUserAccessService} from "../../nao-user-access";
...
export class PageProfileComponent implements OnInit, OnDestroy {
  ...
  public saveInProgress: { userAccount: boolean, companyAccount: boolean } = {
    userAccount: false,
  ...
  public save(type: 'userAccount' | 'companyAccount'): void {
    ...
    // -->Update: user profile
    this.userProfileService.updateAccountData(type, {
      data,
      docId: NaoUserAccessData.userId.getValue()
    }).subscribe(res => {
      if (res && res.ok) {
        // -->Get: promise based on type
        const pro$ = type === 'userAccount' ? this.naoUsersService.refreshSessionData({
          docName: 'doc',
          cfpPath: 'users/users'
        }) : this.appService.getAccountDataInformation();
